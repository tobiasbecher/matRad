function [dvh,qi] = matRad_indicatorWrapperMixed(cst,pln,resultGUI,refGy,refVol)
...
if isfield(resultGUI,'RBExD')
    doseCube = 0;
    for i = 1:numel(pln)
        doseCube = doseCube + pln.originalPlans.numOfFractions(i)*resultGUI{i}.RBExD;
    end
else
    doseCube = 0;
    for i = 1:numel(pln.originalPlans)
        doseCube = doseCube + pln.originalPlans(i).numOfFractions*resultGUI{i}.physicalDose;
    end
end
