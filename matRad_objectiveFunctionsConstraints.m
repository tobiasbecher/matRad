function fIndv = matRad_objectiveFunctionsConstraints(optiProb,w,dij,cst)
    % calculate function values of objectives and objective constraints
    %
    % call
    ...
    % initialize TODO: MAYBE PREDETERMINE LENGTH TO SAVE STORAGE
    fIndv = [];
    ...
    for  i = 1:size(cst,1)
        ...
        if ~isempty(cst{i,4}{1}) && ( isequal(cst{i,3},'OAR') || isequal(cst{i,3},'TARGET') )
            ...
            for j = 1:numel(cst{i,6})
                ...
                if isa(objective,'DoseObjectives.matRad_DoseObjective') || isequal(objective.name,'Objective Constraint')
                    % rescale dose parameters to biological optimization quantity if required
                    if isequal(objective.name,'Objective Constraint') %maybe change order here (could be checked in outer loop, so it is not checked twice)
