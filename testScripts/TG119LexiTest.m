...
%%
cst{1,6}{1} = DoseObjectives.matRad_MeanDose(1,15);
cst{1,6}{2} = DoseObjectives.matRad_SquaredOverdosing(3,10);
cst{3,6}{1} = DoseObjectives.matRad_MeanDose(2,20);
cst{2,6}{2} = DoseObjectives.matRad_SquaredDeviation(4,50);
cst{3,6}{2} = DoseObjectives.matRad_SquaredOverdosing(5,18);
%cst{2,6}{3} =  DoseObjectives.matRad_SquaredUnderdosing(5,54);
%%
[resultGUI,resultGUIs,resultGUIs2,cstL] = matRad_2pecOptimization(dij,cst,pln);

%%
figure
plot(resultGUIs{1}.usedOptimizer.wResult)
hold on
plot(resutlGUI.usedOptimizer.wResult)
...
%%
[PriorityList,cstIt] = matRad_PriorityList(cst);

[cstIt,PriorityList,idx] = matRad_AddObjectiveToCst(cstIt,PriorityList);
[cstIt,PriorityList,idx] = matRad_UpdateCst(cstIt,PriorityList,idx,100)
%%
%cst{1,6} = [];
%cst{2,6}{1} = DoseConstraints.matRad_ObjectiveConstraint(cst{2,6}{1},4.49e-6,1e-5)
%cst{3,6} = [];
...
%%
%cst{2,6}{1} = DoseConstraints.matRad_MinMaxDose(47,53)
...
load('TG119_MOD.mat');
%%
%cst{1,6}{1} = DoseConstraints.matRad_ObjectiveConstraint(DoseObjectives.matRad_MeanDose(100,resultGUI.fopt*30*1.03));%mean dose achieved
cst{1,6}{1} = DoseConstraints.matRad_MinMaxMeanDose(0,resultGUI.fopt*30*1.03)
%
...
resultGUI2 = matRad_fluenceOptimization(dij,cst,pln,resultGUI.w);
%%
resultGUI1 = resultGUI
%%
resultGUI = resultGUI2
%%
%resultGUI = resultGUI1
%%
matRadGUI
